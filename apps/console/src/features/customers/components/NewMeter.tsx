...
export default function NewMeter({
  ...
  const meterAssignMutation = useMutation({
    mutationFn: async (reqData) => {
      return axiosInstance.post("/meters", reqData);
    },
    onError: (error: any) => {
  ...
  const onSubmit = (data: MeterForm) => {
    ...
    meterAssignMutation.mutate(reqData as any);
  };
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate>
        ...
        <div className="mt-12">
          <Button
            type="submit"
            text="Submit"
            isLoading={meterAssignMutation.isLoading}
            width="150px"
          />
        </div>
      </form>
